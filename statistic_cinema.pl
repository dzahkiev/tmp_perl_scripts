...
$cinema->{other} = $DB->select(
	"select
		h.id hall, h.capasity capasity, c.id cinema from hall h
	join cinema c on h.cinema_id = c.id
	where
		$condition_cinema and $condition_hall and
		(c.cinema_network_id is null or c.cinema_network_id=0)"
);
...
my $path = join '/', $conf->{path}->{tmp}, 'statistic_cinema_halls.xls';
my $ss = Spreadsheet::WriteExcel::Simple->new;

my $title = {
	top12        => 'ТОП 12',
	except_top12 => 'Остальные сети',
	other        => 'Независимые кинотеатры',
};
